...
for (const [label, path] of [['cutouts', CUTOUTS], ['report', REPORT], ['onsets', ONSETS], ['song', SONG]]) {
  if (!existsSync(path)) {
    console.error(`missing ${label}: ${path}`);
    console.error('run segment.py / tools/onsets.mjs first.');
    process.exit(1);
  }
}
...
writeFileSync(
  ...
  JSON.stringify({ song: 'song.wav', onsets: 'onsets.json', source: basename(SONG), cats: kept }, null, 2) + '\n',
);
...
console.log(
  `\nstaged ${kept.length} cats + ${analysis.count} onsets → public/viz/` +
